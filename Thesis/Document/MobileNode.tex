...
This application, is specially designed to work in a scenario together with the mobile node but it may work in any \ac{LAN}. The application basically streams the video and audio captured by the smartphone camera throughout the \ac{LAN} and any client within the network can receive it. So, we have two separated entities acting in this scenario: the app which is the server and a computer which is the client. 
