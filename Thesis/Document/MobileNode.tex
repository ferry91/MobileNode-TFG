...
\begin{itemize}
	...
	\item The firmware is different at this point, the main problem  (as we mentioned before) is that every team develops its own firmware with the features they think are better. Therefore, for these people who is not used to this projects it can be a problem, being that they do not really know the difference among the different firmwares. It would be important to, somehow, standardize all the firmwares in one. Despite the fact that every deployment can have different needs, a single firmware may be able to be configured in different ways to reach that needs without having to develop a new one. 
